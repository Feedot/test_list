...
class ItemsList extends Component {
  ...
  onSubmit = e => {
    let { data, addNewItem } = this.props,
      newId = data[data.length - 1].id,
      id = ++newId,
      name = this.input.value,
      sameItem = data.reduce(
        (res, { name }) => (res = name === this.input.value && !res),
        false
      );
  ...
  componentDidUpdate() {
    this.state.name && setTimeout(() => this.setState({ name: false }), 3000);
  }
    ...
    return (
      <div>
        ...
        {data.length &&
          data.map((data, key) => (
            <Item
              {...{key,data, toogleItem, deleteItem,
                num: key,
